...
namespace DataProcess.YdProcess
{
    public class Helper
    {
        ...
        private static void SaveCollectCache(int ledger, string lpszDbVarName, string lpszVal, DateTime lpszdateTime, UInt32 dwUserData)
        {
            ...
            if (collect.Module_id == 0) return;/*无数据内容*/
            ...
            if (cmd != null && cmd.FunType.Equals(V0Fun.Ssr.ToString()) && pro.Project.Equals("YdXip"))
            {
                if (CommFunc.ConvertDBNullToInt32(lpszVal) == 0)
                {/*合闸*/
                    if (isFirst == true || collect.IsRn == 1)
                    {/*检查是否欠费*/
                        DataProcessBLL bll = new DataProcessBLL(ledger, Config.Uid);
                        DataTable dtQf = bll.GetIsQiFei(cmd.Module_id);
                        if (dtQf.Rows.Count > 0)
                        {/*欠费拉闸*/
                            CommandVModel reset = cmd.Clone();
                            reset.DataValue = "1";
                            reset.IsNDb = true;
